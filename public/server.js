   ...
   18
   19 // Fix for Cross-Origin-Opener-Policy blocking Google Sign-In popup
   20 app.use((req, res, next) => {
   21     res.setHeader("Cross-Origin-Opener-Policy", "unsafe-none");
   22     res.setHeader("Cross-Origin-Embedder-Policy", "unsafe-none");
   23     res.setHeader("Referrer-Policy", "no-referrer-when-downgrade");
   24     next();
   25 });
   26
   27 app.use(cors());
   ...
   32
   33 // Configure dotenv to load .env from the project root
   34 dotenv.config({ path: path.resolve(__dirname, '.env') });
   35
   36
   37 // --- ENV & SETUP CHECK ---
   38 const isConfiguredBase = process.env.DB_HOST && process.env.JWT_SECRET && process.env.DB_USER && process.env.DB_NAME && process.
      env.ENCRYPTION_KEY && process.env.GOOGLE_CLIENT_ID;
   39 let isNextcloudConfigured = false;
   40
   41 let pool = null;
   42 let mailer = null;
   43 const JWT_SECRET = process.env.JWT_SECRET;
   44 let googleClient = null;
   45 let webdavClient = null;
   46 let musicWebdavClient = null;
   47 let genAI = null;
   48
   49 // --- HELPER: Robust AI JSON Parser ---
   50 const parseAIResponse = (text) => {
   51     let cleanedText = text;
   52
   53     // 1. Aggressively remove Markdown code blocks, if any
   54     const codeBlockRegex = /```(?:json|javascript|typescript|txt)?\n([\s\S]*?)\n
  `/g;
      ...
      try {
          ...
          try {
              return JSON.parse(correctedText);
                          ...
                          console.warn("AI response could not be parsed into valid JSON after all attempts. Returning empty object.", e1.message, e2.message);
              return {};
          }
      }
  };
  ...
  const initDB = async () => {
      ...
      try {
          ...
          `);
          // Other tables... (simplified for brevity in this re-creation, assuming they exist or will be created on first use if      
  needed, typically schema migration is separate)
  ...
  process.env.NEXTCLOUD_MUSIC_SHARE_PASSWORD),
      ...
      const isOverallOk = Object.values(checks).every(check => check.status === 'ok' || (check.status === 'misconfigured' &&
  !check.configured));
  ...
  app.post('/api/register', async (req, res) => {
      ...
      try {
          ...
          const [result] = await pool.query(
              'INSERT INTO users (sid, email, full_name, password_hash, profile_photo, is_verified, verification_token,
  verification_token_expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          ...
          await pool.query('INSERT INTO user_configs (user_id, config) VALUES (?, ?)', [result.insertId,
  ...
  app.post('/api/verify-email', async (req, res) => {
      ...
      try {
          ...
          await pool.query('UPDATE users SET is_verified = TRUE, verification_token = NULL, verification_token_expires_at = NULL WHERE
  id = ?', [user.id]);
  ...
  async function getUserData(userId) {
      const [userRows] = await pool.query('SELECT id, sid, email, full_name as fullName, profile_photo as profilePhoto, role,
  api_token as apiToken, last_seen FROM users WHERE id = ?', [userId]);
  ...
  app.post('/api/schedule-items', authenticateToken, async (req, res) => {
      ...
      // Simplification: We'll delete based on the logical ID inside JSON and insert new, or use ON DUPLICATE KEY UPDATE if we map    
  IDs.
  ...
  app.post('/api/config', authenticateToken, async (req, res) => {
      ...
      await pool.query('INSERT INTO user_configs (user_id, config) VALUES (?, ?) ON DUPLICATE KEY UPDATE config = ?', [req.user.id,   
  ...
  app.post('/api/push/subscribe', authenticateToken, async (req, res) => {
      ...
      try {
          await pool.query('INSERT INTO push_subscriptions (user_id, subscription) VALUES (?, ?) ON DUPLICATE KEY UPDATE subscription 
  = ?', [req.user.id, JSON.stringify(subscription), JSON.stringify(subscription)]);
  ...
  app.post('/api/doubts', authenticateToken, async (req, res) => {
      ...
      await pool.query(
          'INSERT INTO doubts (id, user_sid, question, question_image, created_at, author_name, author_photo) VALUES (?, ?, ?, ?,     
  NOW(), ?, ?)',
          [doubtId, req.user.sid, question, question_image, req.user.fullName || 'User', req.user.profilePhoto] // Need fullname in   
  token or fetch
      );
  ...
  app.get('/api/admin/students', authenticateToken, async (req, res) => {
      if (req.user.role !== 'admin') return res.sendStatus(403);
      const [users] = await pool.query('SELECT id, sid, email, full_name as fullName, profile_photo as profilePhoto, last_seen, role  
  FROM users');
      res.json(users);
  });
  ...
  app.post('/api/admin/settings', authenticateToken, async (req, res) => {
      ...
      await pool.query('INSERT INTO global_settings (setting_key, setting_value) VALUES (?, ?) ON DUPLICATE KEY UPDATE setting_value =
  ?', [key, value, value]);
      res.json({ success: true });
  });
  ...
  app.post('/api/ai/parse-text', authenticateToken, async (req, res) => {
      ...
      try {
          const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
          const prompt = Parse this text into a JSON object with keys 'schedules', 'exams', or 'practice_test' based on content. Text:
  ${text};
