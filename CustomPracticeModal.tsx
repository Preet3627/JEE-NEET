              ...
              {activeTab === 'ai' && (
                  <div className="space-y-4">
                    ...
                    <div className="bg-gray-900/50 p-3 rounded-lg border border-gray-700/50 space-y-2">
                        <h4 className="text-sm font-bold text-cyan-400">Question Types</h4>
                        <div className="flex flex-wrap gap-x-4 gap-y-2">
                            {(['MCQ', 'NUM', 'MULTI_CHOICE'] as const).map(type => (
                                <label key={type} className="flex items-center text-sm text-gray-300">
                                    <input 
                                        ...
                                        className="w-4 h-4 rounded text-cyan-600 bg-gray-700 border-gray-600 mr-2" 
                                    />
                                    {type === 'MCQ' ? 'MCQ' : type === 'NUM' ? 'Numerical' : 'Multi-Correct'}
                                </label>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-900/50 p-3 rounded-lg border border-gray-700/50 space-y-2">
                        <div className="flex items-center justify-between">
                            <h4 className="text-sm font-bold text-cyan-400">Previous Year Questions (PYQ)</h4>
                            <label className="flex items-center text-sm text-gray-300">
                                <input 
                                    type="checkbox" 
                                    checked={aiIsPYQ} 
                                    onChange={e => setAiIsPYQ(e.target.checked)} 
                                    className="w-4 h-4 rounded text-cyan-600 bg-gray-700 border-gray-600 ml-2" 
                                />
                                Only PYQs
                            </label>
                        </div>
                        {aiIsPYQ && (
                            <>
                                <label className="text-xs font-bold text-gray-400 block mt-2">Filter by Chapters (Optional)</label>
                                <div className="max-h-24 overflow-y-auto p-2 border border-gray-700 rounded bg-gray-800/50">
                                    {availableChapters.length > 0 ? (
                                        <div className="grid grid-cols-2 gap-2 text-sm">
                                            {availableChapters.map(chapter => (
                                                <label key={chapter} className="flex items-center text-gray-300">
                                                    <input 
                                                        type="checkbox" 
                                                        checked={aiPYQChapters.includes(chapter)} 
                                                        onChange={e => handlePYQChapterChange(chapter, e.target.checked)} 
                                                        className="w-4 h-4 rounded text-cyan-600 bg-gray-700 border-gray-600 mr-2" 
                                                    />
                                                    {chapter}
                                                </label>
                                        ...
                                        <p className="text-xs text-gray-500">No specific chapters found for this subject in the knowledge base.</p>
                                    )}
                                </div>
                            </>
                        )}
