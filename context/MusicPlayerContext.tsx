...
export const MusicPlayerProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        let drop: HTMLAudioElement | null = null;
        if (!djDropAudioRef.current) {
            try {
                drop = new Audio(DJ_DROP_URL);
                drop.crossOrigin = "anonymous";
                drop.addEventListener('error', (e) => console.error("DJ Drop Audio Element Error", e));
                djDropAudioRef.current = drop;
            } catch (e) {
                console.error("Failed to create DJ Drop audio element.", e)
            }
        }
